...
from mjlab.asset_zoo.robots.xterra_m2_metal.m2_metal_constants import (
  M2_METAL_ACTION_SCALE,
  M2_METAL_ROBOT_CFG,
)
...
@dataclass
class xTerraM2MetalRoughEnvCfg(LocomotionVelocityEnvCfg):
  def __post_init__(self):
    super().__post_init__()

    self.scene.entities = {"robot": replace(M2_METAL_ROBOT_CFG)}
    ...
    # Actions.
    self.actions.joint_pos.scale = M2_METAL_ACTION_SCALE
...
@dataclass
class xTerraM2MetalRoughEnvCfg_PLAY(xTerraM2MetalRoughEnvCfg):
  def __post_init__(self):
    super().__post_init__()
