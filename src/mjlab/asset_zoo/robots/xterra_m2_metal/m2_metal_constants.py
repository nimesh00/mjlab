...
M2_METAL_XML: Path = (
  MJLAB_SRC_PATH / "asset_zoo" / "robots" / "xterra_m2_metal" / "xmls" / "m2_metal.xml"
)
assert M2_METAL_XML.exists()


def get_assets(meshdir: str) -> dict[str, bytes]:
  assets: dict[str, bytes] = {}
  update_assets(assets, M2_METAL_XML.parent / "assets", meshdir)
  return assets


def get_spec() -> mujoco.MjSpec:
  spec = mujoco.MjSpec.from_file(str(M2_METAL_XML))
...
M2_METAL_HIP_ACTUATOR_CFG = ActuatorCfg(
...
M2_METAL_KNEE_ACTUATOR_CFG = ActuatorCfg(
...
M2_METAL_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    M2_METAL_HIP_ACTUATOR_CFG,
    M2_METAL_KNEE_ACTUATOR_CFG,
  ),
  soft_joint_pos_limit_factor=0.9,
)

M2_METAL_ROBOT_CFG = EntityCfg(
  init_state=INIT_STATE,
  collisions=(FULL_COLLISION,),
  spec_fn=get_spec,
  articulation=M2_METAL_ARTICULATION,
)

M2_METAL_ACTION_SCALE: dict[str, float] = {}
for a in M2_METAL_ARTICULATION.actuators:
  e = a.effort_limit
  s = a.stiffness
  ...
  for n in names:
    if n in e and n in s and s[n]:
      M2_METAL_ACTION_SCALE[n] = 0.25 * e[n] / s[n]
...
if __name__ == "__main__":
  ...
  robot = Entity(M2_METAL_ROBOT_CFG)
