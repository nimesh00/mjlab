...
DHAV1_XML: Path = (
  MJLAB_SRC_PATH / "asset_zoo" / "robots" / "xterra_dhav1" / "xmls" / "dhav1.xml"
)
assert DHAV1_XML.exists()


def get_assets(meshdir: str) -> dict[str, bytes]:
  assets: dict[str, bytes] = {}
  update_assets(assets, DHAV1_XML.parent / "assets", meshdir)
  return assets


def get_spec() -> mujoco.MjSpec:
  spec = mujoco.MjSpec.from_file(str(DHAV1_XML))
...
# Rotor inertia. (Need to update it for A3)
...
DHAV1_HIP_ACTUATOR_CFG = ActuatorCfg(
...
DHAV1_KNEE_ACTUATOR_CFG = ActuatorCfg(
...
DHAV1_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    DHAV1_HIP_ACTUATOR_CFG,
    DHAV1_KNEE_ACTUATOR_CFG,
  ),
  soft_joint_pos_limit_factor=0.9,
)

DHAV1_ROBOT_CFG = EntityCfg(
  init_state=INIT_STATE,
  collisions=(FULL_COLLISION,),
  spec_fn=get_spec,
  articulation=DHAV1_ARTICULATION,
)

DHAV1_ACTION_SCALE: dict[str, float] = {}
for a in DHAV1_ARTICULATION.actuators:
  e = a.effort_limit
  s = a.stiffness
  ...
  for n in names:
    if n in e and n in s and s[n]:
      DHAV1_ACTION_SCALE[n] = 0.25 * e[n] / s[n]
